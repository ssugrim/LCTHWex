...
int *merge_sort(int *numbers, int count, compare_cb cmp)
{
    ...
    int *scratch1 = malloc(((count / 2) + 1) * sizeof(int));
    if(!sratch) die("Memmory error.");

    int *scratch2 = malloc(((count / 2) + 1) * sizeof(int));
    if(!sratch) die("Memmory error.");
    
    lim = (count + 1) / step;  
    memcpy(scratch1, target, lim * 
...
{
    int i =0;
    int *sorted = sort(numbers, count, cmp); //passing around the function pointer
